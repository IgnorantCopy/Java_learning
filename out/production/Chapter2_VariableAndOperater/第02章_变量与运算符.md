...
> 类名、接口名：多单词组成时，所有单词的首字母大写：XxxYyyZzz
  例如：HelloWorld，Variable.StringTest，System等
...
#### 5.3.1 字符串类型：Variable.StringTest

- String不是基本数据类型，属于引用数据类型
- 使用一对`""`来表示一个字符串，内部可以包含0个、1个或多个字符。
- 声明方式与基本数据类型类似。例如：Variable.StringTest str = “尚硅谷”;
...
int num = 10;
boolean b1 = true;
Variable.StringTest s1 = "abc";

Variable.StringTest s2 = s1 + num + b1;
System.out.println(s2);//abc10true

//Variable.StringTest s3 = num + b1 + s1;//编译不通过，因为int类型不能与boolean运算
Variable.StringTest s4 = num + (b1 + s1);//编译通过
```

2、String类型不能通过强制类型()转换，转为其他的类型

```java
Variable.StringTest str = "123";
int num = (int)str;//错误的
...
public class Info {
  public static void main(StringTest[] args) {
    ...
    StringTest phoneNumber = "13112341234";
...
```java
Variable.StringTest str1 = 4;                       //判断对错：
Variable.StringTest str2 = 3.5f + "";               //判断str2对错：
...
public class ArithmeticTest2 {
  public static void main(StringTest[] args) {
    ...
    StringTest str2 = "Java";
    // Variable.StringTest + int --> Variable.StringTest
    System.out.println(str2 + 520); // Java520
    // Variable.StringTest + int + int
    // Variable.StringTest		+ int
    // Variable.StringTest
