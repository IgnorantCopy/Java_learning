...
class AfterJDK5.NumThread implements AfterJDK5.CallableTest {
...
public class AfterJDK5.CallableTest {
  public static void main(String[] args) {
    //3.创建Callable接口实现类的对象
    AfterJDK5.NumThread numThread = new AfterJDK5.NumThread();
...
- `ExecutorService`：真正的线程池接口。常见子类ThreadPoolExecutor
  ...
  - `<T> Future<T> submit(AfterJDK5.CallableTest<T> task)`：执行任务，有返回值，一般又来执行Callable
  - `void shutdown()` ：关闭连接池
...
class NumberThread2 implements CallableTest {
  @Override
  public Object call() throws Exception {
