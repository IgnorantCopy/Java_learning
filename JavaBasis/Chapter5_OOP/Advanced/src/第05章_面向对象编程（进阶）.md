...
```java
class initialize.Person{		// 定义Person类
	private String name ;	
	private int age ;			
	public initialize.Person(String name,int age){	
		this.name = name ;   
		this.age = age ;  
    }
...
```java
public class initialize.inheritance.Student {
    private String name;
    private int age;

    // 无参构造
    public initialize.inheritance.Student() {
//        this("",18);//调用本类有参构造器
    }

    // 有参构造
    public initialize.inheritance.Student(String name) {
        this();//调用本类无参构造器
        this.name = name;
    }
    // 有参构造
    public initialize.inheritance.Student(String name,int age){
...
## 2. 面向对象特征二：继承(inheritance.Inheritance)
...
//SmartPhone：智能手机
public class SmartPhone extends initialize.Phone{
    //重写父类的来电显示功能的方法
	@Override
    public void showNum(){
        //来电显示姓名和图片功能
...
2. 子类重写的方法的返回值类型`不能大于`父类被重写的方法的返回值类型。（例如：initialize.inheritance.Student < initialize.Person）。
...
```java
initialize.Person p = new initialize.inheritance.Student();

Object o = new initialize.Person();//Object类型的变量o，指向Person类型的对象

o = new initialize.inheritance.Student(); //Object类型的变量o，指向Student类型的对象
```
...
public class initialize.Person{
    private Pet pet;
...
public class TestPerson {
    public static void main(String[] args) {
        initialize.Person person = new initialize.Person();
...
```java
initialize.inheritance.Student m = new initialize.inheritance.Student();
m.school = "pku"; 	//合法,Student类有school成员变量
initialize.Person e = new initialize.inheritance.Student(); 
...
class initialize.inheritance.Student extends initialize.Person {
...
class Graduate extends initialize.inheritance.Student{
...
建立InstanceTest 类，在类中定义方法method(initialize.Person e);
在method中:
...
* Object类型的变量与除Object以外的任意引用数据类型的对象都存在多态引用
  ...
  initialize.Person o = new initialize.Person();  
  method(o);
...
* 如果一个类没有特别指定父类，那么默认则继承自Object类。例如：
  ...
  //等价于：
  public class initialize.Person extends Object {
  	...
  }
  ```
...
- 引用类型比较引用(是否指向同一个对象)：只有指向同一个对象时，==才返回true。
  ...
  ```java
  initialize.Person p1=new initialize.Person();  	    
  initialize.Person p2=new initialize.Person();
  if (p1==p2){…}
  ```
...
public class initialize.Person {  
    ...
    @Override
    public String toString() {
        return "initialize.Person{" + "name='" + name + '\'' + ", age=" + age + '}';
    }
}
```
...
public class FinalizeTest {
	public static void main(String[] args) {
		initialize.Person p = new initialize.Person("Peter", 12);
...
```java
public static void main(String[] args) {
	Object obj = new initialize.Person();
...
```java
class com.atguigu.java.initialize.Person
```
